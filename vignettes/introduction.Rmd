...
Easily and effectively visualizing genomic mutation data can help researchers to better understand their genomic data and validate their findings.
This `G3viz` package provides a ready-to-use lollipop-diagram tool which enables users to interactively visualize annotated genomic mutation data in Rstudio or a web browser, without having to know any HTML5/JavaScript technologies.  The `G3viz` package has a number of useful features

- Extract Pfam domain composition for a given HUGO gene
- Automatically classify genomic annotations
- Rich chart options
- Interactive chart legend
- Zoom and brush functions to view every details
- Enable highlighting and labeling findings
...
1. [Quick Start](#quickstart)
    ...
    * [Example 4](#ex4): some predefined chart themes
...
plot.options <- g3Lollipop.options(chart.width = 680,
                                   chart.margin = list(left = 40, right = 10, top = 30, bottom = 25),
                                   chart.background = "transparent",
                                   title.text = "PIK3CA (TCGA-BRCA)", 
                                   title.font = "normal 20px Verdana",
                                   title.color = "steelblue", 
                                   title.alignment = "middle", 
                                   title.dy = "0.3em",
                                   lollipop.track.background = "#ffffee")
...
### <a name="ex3"></a>Example 3: visualize genomic mutation data from `CSV` or `TSV` file

Loading variant data from `CSV` or `TSV` format, this example mimics the lollipop diagram as of [cBioPortal MutationMapper](http://www.cbioportal.org/mutation_mapper).
...
### <a name="ex3"></a>Example 3: visualize mutation data from cBioPortal
